...
def main():
    ...
    if not logging_dir.isfile("executor.json"):
        console.log(
            'Could not find "executor.json" in the given directory. Are you sure it is a '
            "logging folder?",
            style="red",
        )
        return
    ...
    if not world_size:
        console.log(
            "Could not get the total number of tasks, please try relaunching the run.",
            style="red",
        )
        return
    ...
    with console.status("Looking for log files"):
        incomplete_logs = list(
            filter(
                lambda file: int(RANK_FROM_LOG_FILENAME_REGEX.search(file).group(1))
                in incomplete,
    ...
    for incomplete_log in incomplete_logs:
        if not first and not Confirm.ask(
            f"Show next log ([i]{incomplete_log}[/i])?", default=True
        ):
            break
        with console.pager():
