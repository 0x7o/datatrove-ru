...
class GopherQualityFilter(BaseFilter):
    ...
    def filter(self, doc: Document) -> bool | tuple[bool, str]:
        ...
        if (
            ...
            and text.count("#") / n_words > self.max_symbol_word_ratio
        ):
            return False, "gopher_too_many_hashes"
        if (
            self.max_symbol_word_ratio
            and (text.count("...") + text.count("…")) / n_words
            > self.max_symbol_word_ratio
        ):
        ...
        if (
            ...
            and sum(
                s.rstrip().endswith("...") or s.rstrip().endswith("…") for s in lines
            )
            / len(lines)
            > self.max_ellipsis_lines_ratio
        ):
        ...
        if (
            ...
            and sum([any((c.isalpha() for c in w)) for w in words]) / n_words
            < self.max_non_alpha_words_ratio
        ):
        ...
        if (
            self.min_stop_words
            and sum(w in self.stop_words for w in words) < self.min_stop_words
        ):
