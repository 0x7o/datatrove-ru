...
class SentenceDedupSignature(PipelineStep):
    ...
    def save_hashes(self, rank: int, signatures):
        ...
        signatures = np.array(
            signatures,
            dtype=[
                ("hash", self.config.hash_config.np_descr),
                ("doc", "<u4"),
                ("sent", "<u2"),
            ],
        )
    ...
    def get_hashes(
        ...
        sentences = (
            self.tokenizer.sent_tokenize(doc.text)
            if self.config.split_sentences
            else doc.text.splitlines()
        )
        if len(sentences) < self.config.n_sentences:
            return []

        sentences_tokens = [
            simplify_text(sent, self.config.norm_config) for sent in sentences
        ]
        n_sent_grams: list = [
            " ".join(x) for x in ngrams(sentences_tokens, self.config.n_sentences)
        ]
        hashes = [
            (self.hash_fc(n_sent_gram), doc_idx, sentence_idx)
            for sentence_idx, n_sent_gram in enumerate(n_sent_grams)
            if n_sent_gram.strip()
            != ""  # we actually do not want to remove all the \n everywhere
        ]
...
def read_sigs(
    ...
    line_format = (
        f"{config.hash_config.struct_format}IH"
        if not index_file
        else config.hash_config.struct_format
    )
...
class SentenceDedupFilter(PipelineStep):
    ...
    def read_duplicates(self, file: BinaryIO) -> np.ndarray:
        ...
        return read_np_from_file(
            file,
            dtype=np.dtype([("doc", "<u4"), ("sent", "<u2")]),
            is_local_file=self.data_folder.is_local(),
        )

    def remove_dup_sentences(
        self, doc: Document, du_lines: np.ndarray
    ) -> tuple[str, str]:
        sentence_spans = (
            list(self.tokenizer.span_tokenize(doc.text))
            if self.config.split_sentences
            else doc.text.splitlines()
        )
        kept_sentences = []
        ...
        for idx, s in enumerate(sentence_spans):
            ...
            if self.config.split_sentences:
                last_s = s[
                    1
                ]  # use this to include whitespace that is not included in the sentence spans
        if removed_span:
            original_formatted.append("<<<")
            if (
                self.config.min_words_to_remove_span > 0
        ...
        if len(kept_sentences) < len(sentence_spans):
            self.stat_update(
                "removed_sentences", value=len(sentence_spans) - len(kept_sentences)
            )
        self.stat_update("original_sentences", value=len(sentence_spans))
        merge_char = "" if self.config.split_sentences else "\n"
        return merge_char.join(kept_sentences).lstrip(), merge_char.join(
            original_formatted
        )
    ...
    def run(
        ...
        with self.exclusion_writer if self.exclusion_writer else contextlib.nullcontext() as writer:
            for doc_idx, doc in enumerate(data):
                ...
                with self.stats.time_stats:
                    if (
                        dups_doc_i >= len(doc_starts)
                        or all_dups["doc"][doc_starts[dups_doc_i]] > doc_idx
                    ):
                        filtered_text, original_formatted = doc.text, None
                    else:
                        sents_span_l, sents_span_r = (
                            doc_starts[dups_doc_i],
                            doc_starts[dups_doc_i + 1]
                            if dups_doc_i + 1 < len(doc_starts)
                            else None,
                        )
                ...
                if (
                    ...
                    or (
                        (
                            ...
                            or len(self.tokenizer.word_tokenize(filtered_text))
                            >= self.config.min_doc_words
                        )
                        and (
                            # min num sentences
                            ...
                            or len(
                                split_into_parts(
                                    filtered_text, SPLIT_TEXT_SENTENCES, self.language
                                )
                            )
                            >= self.config.min_num_sentences
                        )
                    )
                ) and filtered_text:  # can not be completely empty  # document is kept
                    self.update_doc_stats(doc)
                    if not filtered_text == doc.text and writer:
                        writer.write(
                            dataclasses.replace(doc, text=original_formatted), rank=rank
                        )
...
class SentenceDedupBuildIndex(PipelineStep):
    ...
    def run(self, data: DocumentsPipeline = None, rank: int = 0, world_size: int = 1):
        assert (
            world_size == 1
        ), "SentenceDedupBuildIndex can only run on a single worker."
        with self.stats.time_stats:
            sig_files = self.data_folder.list_files(
                glob_pattern=ExtensionHelperSD.stage_1_signature
            )
            sig_readers = [
                read_sigs(
                    file, file_i, self.config, lines_to_buffer=self.lines_to_buffer
                )
                for file_i, file in enumerate(self.data_folder.open_files(sig_files))
            ]
